...
This module provides spawn_bundle() for orchestration-level session spawning
with features beyond what PreparedBundle.spawn() offers:
- Bundle resolution (URI, Bundle, or PreparedBundle)
- Configuration inheritance (providers, tools, hooks) with fine-grained control
- Infrastructure wiring (resolvers, sys.path, cancellation)
- Session persistence (optional)
- Background execution (optional)
- Event routing integration

This lives in the orchestration bundle because it represents orchestration
policy built on top of foundation's mechanism (PreparedBundle.spawn()).
"""
